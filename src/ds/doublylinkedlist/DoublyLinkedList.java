...
public class DoublyLinkedList {
	...
	public void insertFirst(int data) {
		...
		if(isEmpty()) {
			last = newNode; //if empty, last will refer to the new Node bein created
		} else {
	...
	public Node deleteFirst() {
		...
		first = first.next; // we are assigning the reference of the node following the old first node 
		                    	//to the first field of the linked list object
	...
	public boolean insertAfter(int key, int data) {
		...
		if(current == last) {
			...
			newNode.next = current.next; // new node's next field should point to the node ahead of 
										 	//the current node's next field
			current.next.previous = newNode; // the node ahead of current, it's previous field should point 
											 	//to newNode
		}
